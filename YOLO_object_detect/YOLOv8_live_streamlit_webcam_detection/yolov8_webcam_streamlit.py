...
st.write("Runs entirely in your browser tab (webcam required). Press **Stop** to end.") 

col1, col2, col3 = st.columns(3) 
model_name = col1.selectbox("Model", ["yolov8n.pt", "yolov8s.pt", "yolov8m.pt", "yolov8l.pt", "yolov8x.pt"], index=0) 
conf = col2.slider("Confidence threshold", 0.05, 0.90, 0.25, 0.01) 
device = col3.selectbox("Device hint", ["auto", "cpu", "cuda"], index=0) 
classes_text = st.text_input("Limit to classes (comma-separated class names, optional)", value="") 
...
class YOLOProcessor(VideoProcessorBase): 
    def recv(self, frame): 
        ...
        results = model.predict(img, conf=conf, verbose=False, device=(None if device=="auto" else device), classes=CLASS_NAME_TO_ID) 
